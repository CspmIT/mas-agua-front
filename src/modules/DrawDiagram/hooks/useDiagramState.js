...
export const useDiagramState = () => {
  ...
  const [deletedItems, setDeletedItems] = useState({ lines: [], texts: [], images: [], polylines: [], panels: [], widgets: [] });
  ...
  const handleDeleteElement = useCallback((id) => {
    ...
    if (typeof id === 'string' && id.includes('-')) {
      ...
      if (!isNaN(numericId)) {
        const typeKey = idStr.includes('text') ? 'texts' : idStr.includes('image') ? 'images' : idStr.includes('poly') ? 'polylines' : idStr.includes('panel') ? 'panels' : idStr.includes('widget') ? 'widgets' : 'lines';
